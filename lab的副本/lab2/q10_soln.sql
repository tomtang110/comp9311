...
create table Departments (
	...
	manager     char(11) not null unique
	            constraint ValidEmployee references Employees(tfn) DEFERRABLE INITIALLY DEFERRED,
	primary key (id)
);

alter table Employees
	add column worksIn char(3) not null
		constraint ValidDepartment references Departments(id) DEFERRABLE INITIALLY DEFERRED;
